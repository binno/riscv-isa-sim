...
pmpaddr_csr_t::pmpaddr_csr_t(processor_t* const proc, const reg_t addr, const size_t idx, bool is_spmp):
  csr_t(proc, addr),
  val(0),
  cfg(0),
  pmpidx(idx),
...
bool pmpaddr_csr_t::match4(reg_t addr) const noexcept {
  if (proc->extension_enabled_const(EXT_SSPMPSW) && is_spmp) {
    if ((state->sspmpswitch->read() & (0x1 << pmpidx)) == false)
      return false;
  }
...
mpmpaddr_csr_t::mpmpaddr_csr_t(processor_t* const proc, const reg_t addr):
  pmpaddr_csr_t(proc, addr, addr - CSR_PMPADDR0, false) {
}

void mpmpaddr_csr_t::verify_permissions(insn_t insn, bool write) const {
...
reg_t mpmpaddr_csr_t::read() const noexcept {
...
bool mpmpaddr_csr_t::unlogged_write(const reg_t val) noexcept {
...
bool mpmpaddr_csr_t::access_ok(access_type type, reg_t mode, bool hlvx) const noexcept {
...
bool mpmpdeleg_csr_t::unlogged_write(const reg_t val) noexcept {
  ...
  bool write_success = false;
  if (max_locked_pmp_index < new_val) {
    proc->n_pmp = new_val;
...
spmpaddr_csr_t::spmpaddr_csr_t(processor_t* const proc, const reg_t addr):
  pmpaddr_csr_t(proc, addr, addr & 0x3F, true) {
}
...
sspmpswitch_csr_t::sspmpswitch_csr_t(processor_t* const proc, const reg_t addr, const reg_t init):
  basic_csr_t(proc, addr, init) {
}

bool sspmpswitch_csr_t::unlogged_write(const reg_t val) noexcept {
  reg_t write_val = 0;
  for (reg_t i = 0; i < (64 - proc->n_pmp); ++i) {
    if (!state->spmpaddr[i]->is_locked() && (val & (0x1 << i)))
      write_val |= (0x1 << i);
  }
