...
class pmpaddr_csr_t: public csr_t {
 public:
  pmpaddr_csr_t(processor_t* const proc, const reg_t addr, const size_t idx, bool is_spmp);
 ...
 private:
  ...
  friend class mpmpaddr_csr_t;
  friend class spmpaddr_csr_t;
  reg_t val;
  uint8_t cfg;
  const size_t pmpidx;
  std::shared_ptr<pmpaddr_csr_t>* pmpaddr;
  bool is_spmp;
};

typedef std::shared_ptr<pmpaddr_csr_t> pmpaddr_csr_t_p;

class mpmpaddr_csr_t: public pmpaddr_csr_t {
 public:
  mpmpaddr_csr_t(processor_t* const proc, const reg_t addr);
...
class spmpaddr_csr_t: public pmpaddr_csr_t {
 public:
...
class sspmpswitch_csr_t: public basic_csr_t {
 public:
  sspmpswitch_csr_t(processor_t* const proc, const reg_t addr, const reg_t init);
 protected:
  virtual bool unlogged_write(const reg_t val) noexcept override;
};
