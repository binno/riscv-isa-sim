...
bool mmu_t::pmp_search(reg_t addr, reg_t len, pmpaddr_csr_t_p* pmpaddr, size_t pmp_num, int &index)
{
  index = NO_IDX;
  ...
  for (size_t i = 0; i < pmp_num; i++) {
    ...
    for (reg_t offset = 0; offset < len; offset += 1 << PMP_SHIFT) {
      ...
      bool match = pmpaddr[i]->match4(cur_addr);
      any_match |= match;
      all_match &= match;
    }
...
bool mmu_t::spmp_ok(reg_t addr, reg_t len, access_type type, reg_t mode)
{
  ...
  int pmp_idx = NO_IDX;
  if (pmp_search(addr, len, proc->state.spmpaddr, 64 - proc->n_pmp, pmp_idx)) {
    if (pmp_idx != NO_IDX)
